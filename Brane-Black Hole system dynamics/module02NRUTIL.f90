...
END INTERFACE
!   Routines returning a location as an integer value (ifirstloc, iminloc are not currently
!   over-loaded and so do not have a generic interface here):
INTERFACE imaxloc
...
END INTERFACE
!    Routines for skew operations on matrices (unit matrix, lower triangle, upper triangle
