...
OPEN(10,file='mQTvf.dat')
OPEN(11,file='convh.dat')
OPEN(12,file='conv2h.dat')
OPEN(13,file='conv4h.dat')
OPEN(14,file='conv8h.dat')
...
IF (MOD(licznik,100).EQ.0) THEN
   mEH=0.5D0*rnext(Nkoniec)*(1.D0+4.D0*fnext(Nkoniec)*gnext(Nkoniec)/(anext(Nkoniec)*anext(Nkoniec))&
    +Qnext(Nkoniec)*Qnext(Nkoniec)/(rnext(Nkoniec)*rnext(Nkoniec)))
   WRITE(10,40) u,v(Nkoniec),Qnext(Nkoniec),mEH
END IF

IF (licznik.EQ.1251) THEN
   DO i=1,Nkoniec
      WRITE(14,40) u,v(i),hnext(i),DSQRT(s1next(i)*s1next(i)+s2next(i)*s2next(i))
   END DO
END IF
!
IF (licznik.EQ.2501) THEN
   DO i=1,Nkoniec
      IF (MOD(i-1,2).EQ.0) WRITE(13,40) u,v(i),hnext(i),DSQRT(s1next(i)*s1next(i)+s2next(i)*s2next(i))
   END DO
END IF
!
IF (licznik.EQ.5001) THEN
   DO i=1,Nkoniec
      IF (MOD(i-1,4).EQ.0) WRITE(12,40) u,v(i),hnext(i),DSQRT(s1next(i)*s1next(i)+s2next(i)*s2next(i))
   END DO
END IF
!
IF (licznik.EQ.10001) THEN
   DO i=1,Nkoniec
      IF (MOD(i-1,8).EQ.0) WRITE(11,40) u,v(i),hnext(i),DSQRT(s1next(i)*s1next(i)+s2next(i)*s2next(i))
   END DO
END IF
